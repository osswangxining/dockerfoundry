...
public class DockerFoundryDecorator extends LabelProvider implements ILightweightLabelDecorator {
		...
		if (element instanceof ModuleServer) {
			...
			if (s != null && CloudServerUtil.isCloudFoundryServer(s)) {
				...
				if (modules != null && modules.length == 1) {
					...
					// }
					if (application != null) {
						decoration.addSuffix(NLS.bind(Messages.DockerFoundryDecorator_SUFFIX_DEPLOYED_AS, application.getName()));
					}
					else {
...
//					}
	...
	private DockerFoundryServer getCloudFoundryServer(IServer server) {
		Object obj = server.getAdapter(DockerFoundryServer.class);
		if (obj instanceof DockerFoundryServer) {
			return (DockerFoundryServer) obj;
		}
		return null;
	}
