...
import cn.dockerfoundry.ide.eclipse.server.ui.internal.UIPart;
import cn.dockerfoundry.ide.eclipse.server.ui.internal.WizardPartChangeEvent;
 ...
 * 
 */
public class DockerFoundryApplicationWizardPage extends PartsWizardPage {

	private Pattern VALID_CHARS = Pattern.compile("[A-Za-z\\$_0-9\\-]+"); //$NON-NLS-1$

	protected static final String DEFAULT_DESCRIPTION = Messages.DockerFoundryApplicationWizardPage_TEXT_SET_APP_DETAIL;
	...
	protected final DockerFoundryServer server;

	protected final DockerFoundryApplicationModule module;

	protected String filePath;

	protected final ApplicationWizardDescriptor descriptor;

	protected final DockerFoundryDeploymentWizardPage deploymentPage;

	public DockerFoundryApplicationWizardPage(DockerFoundryServer server,
			DockerFoundryDeploymentWizardPage deploymentPage, DockerFoundryApplicationModule module,
			ApplicationWizardDescriptor descriptor) {
		super(Messages.DockerFoundryApplicationWizardPage_TEXT_DEPLOY_WIZ, null, null);
		this.server = server;
	...
	protected DockerFoundryApplicationWizard getApplicationWizard() {
		return (DockerFoundryApplicationWizard) getWizard();
	}

	public void createControl(Composite parent) {
		setTitle(Messages.DockerFoundryApplicationWizardPage_TITLE_APP_DETAIL);
	...
	protected Composite createContents(Composite parent) {
		...
		nameLabel.setText(Messages.DockerFoundryApplicationWizardPage_LABEL_BUILDPACK);
		...
		saveToManifest.setText(Messages.DockerFoundryApplicationWizardPage_BUTTON_SAVE_MANIFEST);
		saveToManifest.setToolTipText(Messages.DockerFoundryApplicationWizardPage_TEXT_SAVE_MANIFEST_TOOLTIP);
		...
		if (!ValueValidationUtil.isEmpty(buildpack)) {
			try {
				...
				if (host == null || host.length() == 0) {
					status = DockerFoundryPlugin.getErrorStatus(Messages.COMMONTXT_ENTER_VALID_URL);
				}
				else {
	...
	protected IStatus getUpdateNameStatus() {
		...
		if (ValueValidationUtil.isEmpty(appName)) {
			status = DockerFoundryPlugin.getStatus(Messages.DockerFoundryApplicationWizardPage_ERROR_MISSING_APPNAME,
					IStatus.ERROR);
		}
		else {
			Matcher matcher = VALID_CHARS.matcher(appName);
			if (!matcher.matches()) {
				status = DockerFoundryPlugin
						.getErrorStatus(Messages.DockerFoundryApplicationWizardPage_ERROR_INVALID_CHAR);
			}
			else {
				ModuleCache moduleCache = DockerFoundryPlugin.getModuleCache();
				ServerData data = moduleCache.getData(server.getServerOriginal());
				Collection<DockerFoundryApplicationModule> applications = data.getExistingCloudModules();
				boolean duplicate = false;

				for (DockerFoundryApplicationModule application : applications) {
				...
				if (duplicate) {
					status = DockerFoundryPlugin
							.getErrorStatus(Messages.DockerFoundryApplicationWizardPage_ERROR_NAME_CONFLICT);
				}
			}
		}
