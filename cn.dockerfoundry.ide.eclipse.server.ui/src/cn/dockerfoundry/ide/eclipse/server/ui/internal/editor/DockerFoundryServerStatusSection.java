 ...
 * @author Terry Denney
 */
public class DockerFoundryServerStatusSection extends ServerEditorSection implements CloudServerListener, IServerListener {
	...
	public void createSection(Composite parent) {
		...
		section.setText(Messages.DockerFoundryServerStatusSection_TEXT_SERV_STAT);
		...
		connectButton = toolkit.createButton(composite, Messages.DockerFoundryServerStatusSection_TEXT_CONN_BUTTON, SWT.PUSH);
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).applyTo(connectButton);
		
		connectButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				Job job = new Job(Messages.DockerFoundryServerStatusSection_JOB_CONN_SERVER) {
					...
					protected IStatus run(IProgressMonitor monitor) {
						try {
							...
							StatusManager.getManager().handle(new Status(Status.ERROR, DockerFoundryServerUiPlugin.PLUGIN_ID, "", e), StatusManager.LOG); //$NON-NLS-1$
		...
		disconnectButton = toolkit.createButton(composite, Messages.DockerFoundryServerStatusSection_TEXT_DISCONN_BUTTON, SWT.PUSH);
