 ...
 * @author Nieraj Singh
 */
public class DockerFoundryDeploymentWizardPage extends AbstractURLWizardPage {
	...
	protected DockerFoundryApplicationWizard wizard;
	...
	public DockerFoundryDeploymentWizardPage(DockerFoundryServer server, DockerFoundryApplicationModule module,
			ApplicationWizardDescriptor descriptor, ApplicationUrlLookupService urlLookup,
			ApplicationWizardDelegate wizardDelegate) {
		super(Messages.DockerFoundryDeploymentWizardPage_TEXT_DEPLOYMENT, null, null);
		this.server = server;
		this.module = module;
	...
	protected void setMemory(String memoryVal) {
		...
		if (memory > 0) {
			...
			status = DockerFoundryPlugin.getErrorStatus(Messages.ERROR_INVALID_MEMORY);
		}
		handleChange(new PartChangeEvent(memoryVal, status, CloudUIEvent.MEMORY));
	}

	public void createControl(Composite parent) {
		setTitle(Messages.DockerFoundryDeploymentWizardPage_TITLE_LAUNCH_DEPLOY);
		setDescription(Messages.DockerFoundryDeploymentWizardPage_TEXT_DEPLOY_DETAIL);
		ImageDescriptor banner = DockerFoundryImages.getWizardBanner(serverTypeId);
		if (banner != null) {
			setImageDescriptor(banner);
		}

		this.wizard = (DockerFoundryApplicationWizard) getWizard();
	...
	protected void createStartOrDebugOptions(Composite parent) {
		...
		String startLabelText = Messages.DockerFoundryDeploymentWizardPage_LABEL_START_APP;
	 ...
	 * 
	 * @see
	 * cn.dockerfoundry.ide.eclipse.server.ui.internal.wizards.PartsWizardPage
	 * #handleChange
