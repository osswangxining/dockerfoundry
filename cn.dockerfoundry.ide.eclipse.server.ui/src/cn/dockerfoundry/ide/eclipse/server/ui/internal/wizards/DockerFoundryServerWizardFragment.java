...
public class DockerFoundryServerWizardFragment extends WizardFragment {

	private DockerFoundryServer cloudServer;

	private DockerFoundryCredentialsPart credentialsPart;
	...
	public Composite createComposite(Composite parent, IWizardHandle wizardHandle) {
		...
		credentialsPart = new DockerFoundryCredentialsPart(cloudServer, new WizardHandleContext(wizardHandle));
...
//		}
	...
	private void initServer() {
		...
		cloudServer = (DockerFoundryServer) server.loadAdapter(DockerFoundryServer.class, null);
		if (cloudServer == null) {
			DockerFoundryPlugin
	...
	private static class ConnectJob extends Job {
		...
		private final DockerFoundryServer originalServer;

		private final IServer server;

		public ConnectJob(DockerFoundryServer originalServer, IServer server) {
			super(Messages.DockerFoundryServerWizardFragment_TEXT_CONNECT_ACCOUNT);
		...
		protected IStatus run(final IProgressMonitor monitor) {
			DockerFoundryServer cf = (DockerFoundryServer) server.loadAdapter(DockerFoundryServer.class, monitor);
...
//				}
				...
				if (connect) {
					DockerFoundryServerBehaviour behaviour = cf.getBehaviour();
					if (behaviour != null) {
						try {
