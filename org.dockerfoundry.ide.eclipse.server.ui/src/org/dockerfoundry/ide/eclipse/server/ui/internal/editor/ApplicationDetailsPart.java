 ...
 ********************************************************************************/
package org.dockerfoundry.ide.eclipse.server.ui.internal.editor;
...
import org.dockerfoundry.ide.eclipse.server.core.internal.ApplicationAction;
import org.dockerfoundry.ide.eclipse.server.core.internal.CloudErrorUtil;
import org.dockerfoundry.ide.eclipse.server.core.internal.CloudFoundryBrandingExtensionPoint;
...
import org.dockerfoundry.ide.eclipse.server.core.internal.client.DeploymentInfoWorkingCopy;
import org.dockerfoundry.ide.eclipse.server.ui.internal.CloudUiUtil;
import org.dockerfoundry.ide.eclipse.server.ui.internal.DebugCommand;
import org.dockerfoundry.ide.eclipse.server.ui.internal.Messages;
import org.dockerfoundry.ide.eclipse.server.ui.internal.actions.DebugApplicationEditorAction;
import org.dockerfoundry.ide.eclipse.server.ui.internal.actions.RemoveServicesFromApplicationAction;
import org.dockerfoundry.ide.eclipse.server.ui.internal.actions.StartStopApplicationAction;
import org.dockerfoundry.ide.eclipse.server.ui.internal.actions.EditorAction.RefreshArea;
import org.dockerfoundry.ide.eclipse.server.ui.internal.wizards.EnvVarsWizard;
...
//				.getServerType().getId());
		...
		if (this.cloudServer.getDockerConnElem() != null) {
			try {
				this.dockerClient = this.cloudServer.getDockerConnElem().getDockerClient();
				String containerId = this.cloudServer.getDockerContainerId();
				ContainerInfo containerInfo = this.dockerClient.inspectContainer(containerId);
				
				dockerContainerInfo = DockerDomainHelper.getDockerInfo(containerInfo);
				if(dockerContainerInfo != null){
					this.appUrls.addAll(dockerContainerInfo.getExposedURLs());
				}
			}
...
//		}
		...
		if(this.dockerContainerInfo != null){
			DockerContainerInfo[] infos = new DockerContainerInfo[1];
			infos[0] = this.dockerContainerInfo;
			instancesViewer.setInput(infos);
		}
...
//		}
		List<String> currentURIs = new ArrayList<String>();
		if(this.appUrls != null){
			for (String appUrl : this.appUrls) {
				currentURIs.add(appUrl + "/" + appModule.getName());
			}
		}
		updateAppUrls(currentURIs);
...
//		});
		...
		Button envVarsButton = createGeneralPushButton(client, Messages.COMMONTXT_VIEW);
						...
						UIJob uiJob = new UIJob(Messages.ApplicationDetailsPart_JOB_EDIT_ENV_VAR) {
							...
							public IStatus runInUIThread(IProgressMonitor monitor) {
								try {
									...
									EnvVarsWizard wizard = new EnvVarsWizard(cloudServer, appModule, infoWorkingCopy, dockerContainerInfo);
...
//	}
	...
	protected Button createGeneralPushButton(Composite parent, String text) {
		...
		GridDataFactory.fillDefaults().align(SWT.LEFT, SWT.CENTER).hint(80, SWT.DEFAULT).applyTo(button);
		return button;
	}
...
//	}
	...
	private void createInstancesSection(Composite parent) {
		...
		int[] columnWidths = new int[] { /*25,*/ 105, 105, 60, 85, /*85, */75 };
...
//
//		if (instanceObject instanceof DockerContainerInfo) {
//
//			DockerContainerInfo stats = ((DockerContainerInfo) instanceObject);
//
//			if (stats != null) {
//				try {
//					CloudFoundryApplicationModule appModule = getExistingApplication();
