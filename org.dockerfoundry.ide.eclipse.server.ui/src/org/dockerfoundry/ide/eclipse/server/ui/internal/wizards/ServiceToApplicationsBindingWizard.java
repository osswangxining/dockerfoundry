...
import org.dockerfoundry.ide.eclipse.server.ui.internal.ICoreRunnable;
import org.dockerfoundry.ide.eclipse.server.ui.internal.Logger;
import org.dockerfoundry.ide.eclipse.server.ui.internal.Messages;
import org.dockerfoundry.ide.eclipse.server.ui.internal.PartChangeEvent;
import org.dockerfoundry.ide.eclipse.server.ui.internal.ServiceToApplicationsBindingPart;
import org.dockerfoundry.ide.eclipse.server.ui.internal.ServiceToApplicationsBindingPart.ApplicationToService;
import org.dockerfoundry.ide.eclipse.server.ui.internal.actions.AddServicesToApplicationAction;
import org.dockerfoundry.ide.eclipse.server.ui.internal.actions.ModifyServicesForApplicationAction;
import org.dockerfoundry.ide.eclipse.server.ui.internal.actions.RemoveServicesFromApplicationAction;
import org.dockerfoundry.ide.eclipse.server.ui.internal.editor.CloudFoundryApplicationsEditorPage;
import org.dockerfoundry.ide.eclipse.server.ui.internal.editor.ServicesHandler;
				...
				CloudUiUtil.runForked(new ICoreRunnable() {
					...
					public void run(IProgressMonitor monitor) throws CoreException {
						...
						if (serviceToApplicationsBindingPart != null && server != null && behaviour != null) {
							...
							DockerApplicationService cloudService = null;

							try {
								// Find the Cloud Service that was selected
								List<DockerApplicationService> cloudServiceList = behaviour.getServices(monitor);
								int lenCloudService = cloudServiceList.size();

								String serviceName = servicesHandler.toString();

								for (int j = 0; j < lenCloudService; j++) {
									DockerApplicationService currService = cloudServiceList.get(j);
									if (currService != null) {
